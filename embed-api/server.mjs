...
import { getProfile, filterProfile } from './profile-store.mjs';
 ...
 *
 * Routes:
 *   GET /embed/:roomId         → HTML embed page (use ?show=name,skills to select elements)
 *   GET /api/profile/:roomId   → JSON profile data (use ?fields=name,skills for partial)
...
const server = http.createServer(async (req, res) => {
    ...
    if (embedMatch) {
        const roomId = decodeURIComponent(embedMatch[1]);
        const showElements = parseElements(params.get('show'));

        try {
            const profile = await getProfile(roomId, RELAY_URL);
            if (!profile) {
    ...
    if (apiMatch) {
        const roomId = decodeURIComponent(apiMatch[1]);
...
<style>
    ...
    <h2>Endpoints</h2>
    <div class="endpoint"><code>GET /embed/:roomId</code> — HTML embed page</div>
    <div class="endpoint"><code>GET /embed/:roomId?show=name,skills</code> — Select elements</div>
    <div class="endpoint"><code>GET /api/profile/:roomId</code> — Raw JSON</div>
    <div class="endpoint"><code>GET /api/profile/:roomId?fields=name,skills</code> — Partial JSON</div>
    ...
    <h2>Quick Start</h2>
    <pre>&lt;iframe src="/embed/YOUR_ROOM_ID?show=avatar,name,skills"
        width="440" height="300" frameborder="0"&gt;&lt;/iframe&gt;</pre>
    <p>Or with the SDK:</p>
    <pre>&lt;script src="/embed.js"&gt;&lt;/script&gt;
&lt;profile-embed room="YOUR_ROOM_ID" show="name,title,skills"&gt;&lt;/profile-embed&gt;</pre>
</body></html>`);
});
